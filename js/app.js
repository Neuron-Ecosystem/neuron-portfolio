...
function openDetailsModal(ach) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div class="docs-grid" id="gallery">
            ${ach.documents?.map(d => `<img src="${d.url}" class="modal-img">`).join('')}
        </div>
        ${getCurrentUser()?.role === 'admin' ? `<button id="delBtn" class="btn btn-danger" style="width:100%; margin-top:20px">Удалить достижение</button>` : ''}
    `;

    // Увеличение фото при клике (Lightbox)
...
function openAddModal(user) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div class="input-group">
            <label>Кто вы на мероприятии *</label>
        ...
        <div class="input-group">
            ...
            <textarea id="aDesc" class="input-field" style="height:100px; resize:none;" placeholder="Расскажите подробности..."></textarea>
        </div>
        <div class="input-group">
            <label>Фото грамот, кубков (обязательно) *</label>
    ...
    document.getElementById('sendBtn').onclick = async () => {
        ...
        if (!title || !desc || files.length === 0) return alert('Заполните все поля и добавьте фото!');
...
// --- ОСТАЛЬНЫЕ СТРАНИЦЫ (Профиль, Админка) ---
    ...
    container.innerHTML = wrap(`
        ...
        <div class="card" style="margin:20px 0; border-left:4px solid var(--primary)">
            <div class="card-body">
                ...
                <p class="meta">${user.class.grade}${user.class.letter} | ${user.role}</p>
            </div>
        </div>
...
export async function renderAdmin(container) {
    ...
    pending.forEach(item => {
        ...
        `;
        div.querySelector('.ok-btn').onclick = async (e) => { e.stopPropagation(); await updateStatus(item.id, 'approved', getCurrentUser().lastName); renderAdmin(container); };
        div.querySelector('.no-btn').onclick = async (e) => { e.stopPropagation(); await updateStatus(item.id, 'rejected', getCurrentUser().lastName); renderAdmin(container); };
...
export async function renderUserProfile(container, userId) {
    ...
    container.innerHTML = wrap(`
        ...
        <div class="card" style="margin:20px 0">
            <div class="card-body"><h1>${info?.userName || 'Профиль'}</h1><p class="meta">${info?.userClass || ''}</p></div>
        </div>
    ...
    users.forEach(u => {
        ...
        card.innerHTML = `
            <div class="card-body">
                ...
                <button class="btn toggle-btn" style="width:100%; margin-top:10px">${u.isActive ? 'Забанить' : 'Разбанить'}</button>
            </div>
        `;
